    ...
    Promise.all([getReportSubs('reimbursement'), getReimMonthlyData()]).then(function (result) {
        ...
        reimData.forEach(function (dayRecord) {
            if (month !== dayRecord.month) {
                frag.appendChild(createElement('div',{className:'hr-sect hr-sect mdc-theme--primary mdc-typography--headline5 mdc-layout-grid__cell--span-12',textContent:moment(`${dayRecord.month + 1}-${dayRecord.year}`,'MM-YYYY').format('MMMM YYYY')}))
            };
            month = dayRecord.month;
            let totalAmount = 0;
            const body = createElement('tbody',{className:'data-table__content'})
...
const card = createElement('div',{className:'mdc-card reim-card mdc-layout-grid__cell--span-12'})
const primary = createElement('div',{className:'mdc-card__primary-action'})
const demo = createElement('div',{className:'demo-card__primary'})
const monthlyDateCont = createElement('div',{className:'month-date-cont'})

const dayDiv = createElement('div',{className:'day',textContent:day})
const date = createElement('div',{className:'date',textContent:dayRecord.date})
const headingContainer = createElement('div',{className:'heading-container'})
...
const primaryRow = createElement('tr',{className:'data-table__header-row'})
const claims = createElement('th',{className:'data-table__header-cell ',textContent:'Claim Type'})
const amount = createElement('th',{className:'data-table__header-cell cell-numeric',textContent:'Amount'})
const status = createElement('th',{className:'data-table__header-cell cell-numeric',textContent:'Status'})
