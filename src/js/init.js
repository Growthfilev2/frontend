          ...
          newWorker.addEventListener("statechange", (e) => {
            ...
            if (newWorker.state === "activated") {
              ...
              firebase.auth().onAuthStateChanged((user) => {
                ...
                redirect(
                  `/login.html${deepLink ? `?${deepLink.toString()}` : ""}`
                );
              });
            }
          });
...
const loadApp = () => {
  ...
  if (!window.Worker && !window.indexedDB) {
    const incompatibleDialog = new Dialog(
      "App Incompatiblity",
      "OnDuty is incompatible with this device"
    ).create();
...
function initProfileView() {
  ...
  if (!auth.displayName)
    return redirect(`/profile_edit.html?askPhoto=1&new_user=${isNewUser()}`);
  redirect("/home");
}
...
function checkEmptyIdProofs(record) {
  ...
  keys.forEach(function (key) {
    if (
      !record.idProof[key].number ||
      !record.idProof[key].front ||
      !record.idProof[key].back
    ) {
      isEmpty = true;
      return;
    }
...
function createSubscriptionObjectStore(db) {
  ...
  subscriptions.createIndex("validSubscription", [
    "office",
    "template",
    "status",
  ]);
