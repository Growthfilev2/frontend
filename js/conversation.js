...
function renderShareIcon(record) {
  ...
  document.getElementById("share-btn").onclick = function (e) {
    initDrawer.open('.share--drawer');
    renderShareDrawer(record.activityId);
  };

}


function renderShareDrawer(activityId) {
  const TIME_OUT = 500;

  const contactTextField = mdc
    .textField
    .MDCTextField
    .attachTo(document.querySelector('.contact--text-field'));

  function showClickedContact() {
    setTimeout(function () {
      const contacts = document.querySelectorAll('#contacts--container  li');
      [...contacts, ].forEach(function (li) {
        li.addEventListener('click', function (e) {
          contactTextField.value = e.target.dataset.num;
        });
      });
    }, TIME_OUT);
  }
      ...
      .onkeyup = function (e) {
        ...
        const boundKeyRange = IDBKeyRange
          ...
          userObjectStore.openCursor(boundKeyRange).onsuccess = function (cursorEvent) {
            ...
            matchedContactNumbers.push(cursor.primaryKey);
            assigneeListUI(cursor.value)
