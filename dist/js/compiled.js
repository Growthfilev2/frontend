...
function getActivityDataForList(activity, value, currentLocation) {
  return new Promise(function (resolve, reject) {
    var secondLineParent = document.createElement('div');
    secondLineParent.style.marginTop = '10px';

    var secondLineVenue = document.createElement('span');
    secondLineVenue.className = 'mdc-list-item__secondary-text venue-secondline';

    var secondLineSchedule = document.createElement('span');
    ...
    activity.get(value.activityId).onsuccess = function (event) {
      ...
      var venueSpan = void 0;
      var dateSpan = generateLastestSchedule(schedules, value.createdTime);
...
function generateLastestSchedule(schedules, createdTime) {
  var validSchedules = removeEmptyObjects(schedules, 'startTime', 'endTime');
  var length = validSchedules.length;
  ...
  var ascendingOrder = sortDatesInAscendingOrderWithPivot(currentTime, validSchedules);
...
function getTimeTypeForMultipleSchedule(pivot, dates) {
  ...
  var index = duplicate.indexOf(pivot);
...
function createActivityIconDom() {
  ...
  var fab = new Fab('add');
  var chooseSubscription = fab.getButton();
...
function isLocationStatusWorking() {
  ...
  if (!navigator.onLine) {
    var connectionDialog = new Dialog('BROKEN INTERNET CONNECTION', 'Make Sure You have a working Internet Connection').create();
