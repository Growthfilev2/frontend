...
var loadProfileData = function loadProfileData(user) {
  ...
  req.onsuccess = function (e) {
    ...
    db.transaction('children').objectStore('children').index('employees').get(user.phoneNumber).onsuccess = function (e) {
      ...
      ;
      document.getElementById('employee-at').textContent = record.attachment.Designation && record.attachment.Designation.value ? "".concat(record.attachment.Designation.value, ", ").concat(record.office) : record.office;
...
var calculateProfileVerification = function calculateProfileVerification(rootRecord) {
  var fields = [hasBankAccount(rootRecord), rootRecord.pan, rootRecord.aadhar];
